...
class BuiltinChannelSeedTest {
    ...
    fun buildInsertCommand_createsSingleInsertStatementForAllBuiltins() {
        ...
        assertEquals(0, args[9])
        assertEquals(BuiltinChannelType.DOUYIN.url, args[10])
        assertEquals("抖音", args[11])
        assertEquals(1233L, args[15])
        assertEquals(1233L, args[16])
        assertEquals(0, args[17])
        assertEquals(1, args[18])
        assertEquals(0, args[19])
    }
}
