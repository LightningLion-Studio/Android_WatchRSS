...
data class LlmTokenUsageStatisticsPojo(
...
data class LlmTokenUsageByProviderPojo(
    val provider: String = "",
    val model: String = "",
    val calls: Long = 0L,
    val totalTokens: Long? = 0L,
...
data class LlmTokenUsageDailyPojo(
...
@Dao
interface LlmTokenUsageDao {

    @Insert
    suspend fun insert(entity: LlmTokenUsageEntity)

    @Query("DELETE FROM llm_token_usage")
    suspend fun deleteAll()

    @Query("SELECT * FROM llm_token_usage ORDER BY createdAt DESC LIMIT :limit")
    fun observeRecent(limit: Int): Flow<List<LlmTokenUsageEntity>>
    ...
    """)
    fun observeStatistics(provider: String? = null): Flow<LlmTokenUsageStatisticsPojo>
    ...
    """)
    fun observeByProvider(): Flow<List<LlmTokenUsageByProviderPojo>>
    ...
    ): Flow<List<LlmTokenUsageDailyPojo>>
}
