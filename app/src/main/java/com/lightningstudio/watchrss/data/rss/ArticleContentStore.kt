...
interface ArticleContentStore {
    ...
    fun hasText(marker: String): Boolean
...
class FileArticleContentStore internal constructor(
    ...
    constructor(context: Context) : this(File(context.applicationContext.filesDir, "article_content"))
    ...
    override fun hasText(marker: String): Boolean {
        if (!isMarker(marker)) return false
        return fileFor(marker)?.isFile == true
    }
...
private fun articleContentMarkerFileName(value: String?): String? {
    ...
    return fileName.takeIf { V2_FILE_NAME_REGEX.matches(it) || LEGACY_FILE_NAME_REGEX.matches(it) }
}
...
const val ARTICLE_CONTENT_MARKER_PREFIX = "watchrss-watch-content:"
